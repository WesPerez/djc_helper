...
class DjcHelper:
    ...
    def query_xinyue_team_this_week_award_summary(self) -> str:
        ...
        now = get_now()
        if now.isoweekday() in [5, 6, 7] and len(mine_summary) < 3:
            async_message_box(
    ...
    ark_lottery_sub_act_id_login = "126414_6874f4d4"  # 增加抽卡次数-每日登陆游戏
    ark_lottery_sub_act_id_share = "126408_090a316e"  # 增加抽卡次数-每日活动分享
    ark_lottery_sub_act_id_lucky = "126409_e2f70c39"  # 增加抽卡次数-幸运勇士
    ark_lottery_sub_act_id_draw_card = "126410_86139c8b"  # 抽卡
    ark_lottery_sub_act_id_award_1 = "126411_b2480844"  # 领取奖励-第一排
    ark_lottery_sub_act_id_award_2 = "126412_b4879545"  # 领取奖励-第二排
    ark_lottery_sub_act_id_award_3 = "126413_50922076"  # 领取奖励-第三排
    ark_lottery_sub_act_id_award_all = "126416_4a74be69"  # 领取奖励-十二张
    ark_lottery_sub_act_id_lottery = "126415_cae490c6"  # 消耗卡片来抽奖

    # note: 清空抓包数据，按f5刷新后，搜索  QueryItems  (hack: 其实就是活动链接的 最后一部分)
    ark_lottery_packet_id_card = "52273_fcdc5b07"  # 查询当前卡片数目
    ...
    def dnf_chaohe_wpe(self):
        ...
        def query_is_chaohe() -> bool:
            ...
            # {
            #     "ret": 0,
            #     "msg": "仅限添加了闪闪的超核玩家创建队伍，请联系微信联系闪闪进行身份验证。",
    ...
    def colg_other_act(self):
        # https://hub.bbs.colg.cn/activity/summer_celebration/2026/index.html
        ...
        if now_in_range("2026-06-11 00:00:00", "2026-07-08 23:59:59"):
            res = session.post(
                self.urls.colg_other_act_lottery,
                data=f"aid={self.urls.colg_other_act_id}&pageActivityid=37",
                timeout=10,
            )
            logger.info(color("bold_green") + f"每日签到，结果={res.text}")
            time.sleep(5)

            # fixme: 这里需要补上一个累计签到 3/7/10/14/23/27 的部分
...
"""
    ...
    def dnf_helper_limit_act(self):
        ...
        def query_info() -> tuple[int, int, int, int]:
            ...
            res = raw_res["data"]["user"]

            remainDrawTimes = int(res["remainDrawTimes"])
            fame = int(res["fame"])
            level = int(res["level"])
            experience = int(res["experience"])
    ...
    def dnf_recall_guide(self):
        ...
        self.dnf_recall_guide_op("记录浏览", "553425")
        time.sleep(3)

        self.dnf_recall_guide_op("领取奖励", "553424")
    ...
    def dnf_anniversary_special_act(self):
        ...
        async_message_box(
            "周年庆的新职业预约需要填手机号和验证码，请在稍后弹出的活动页面点击左侧tab最上方的【首页-职业预约】，然后点击【立即预约新职业】，输入手机号和验证码来完成预约流程。预约奖励在6.11后可以领取，到时候运行小助手时将会自动尝试领取~",
            "26.5 新职业预约",
            open_url=get_act_url("周年特别节目"),
            show_once=True,
        )
        async_message_box(
            "游戏内右下角活动图标区域有个小电视，点击后会出现一个界面，点击领取按钮可以领取一份游戏内奖励~",
            "26.5 周年庆游戏内活动",
            show_once=True,
        )
