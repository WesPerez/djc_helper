...
1. 新增 助手限定(逐浪夏日签到) 活动
2. 新增 落地页 活动
3. 如需购买自动更新DLC或按月付费，请在配置工具的【付费相关】标签页自助购买，具体流程请查阅目录中【付费指引.docx】
4. 其他改动及上述功能具体用法，详见README.MD和CHANGELOG.MD以及使用教程/使用文档.docx和教程视频
...
## 活动维护
1. 新增 井盖杯强者之路 活动
2. 新增 助手限定(夏日破浪补给站) 活动

# v22.14.3 2026.6.15
## 活动维护
1. 修复落地页打卡累积签到第一个没领到的问题
2. 增加落地页打卡本身可能有bug，提示手动去打卡的提示
...
2. 新增 DNF闪光杯 活动
3. 新增 WeGame 活动
4. 落地页活动其他几个奖励也轮流领取，避免积分用不完

# v22.12.0 2026.3.12
## 活动维护
1. 新增 落地页 活动
2. 新增 助手限定(狄瑞吉欧气排行榜) 活动
3. 新增 狄瑞吉困难教学 活动的提示
4. 新增 网吧 活动的提示
5. 新增 微信游戏圈 活动的提示
...
## 其他
1. 心悦查询绑定角色加个保底流程，若查询失败后尝试绑定道聚城角色后，仍查询失败，则直接使用道聚城的角色，确保流程正常进行
...
## 其他
1. 调整多进程进程大小配置含义，原本默认的-1改为1/2CPU核心数，并新增-2、-3两个选项，确保默认情况下负载不会太高
2. 更换一个用于检测skey的活动

## 文案调整
1. 特殊处理下支付后立刻点击支付按钮时弹出的无效的QQ的难以理解的提示
...
## 排版调整
1. 调整运行结束时的账号概览与付费信息的顺序，并精简末尾的一些其他信息，使获取关键信息更方便

## 其他
1. 更新github镜像
2. 自动更新通过github镜像下载全量包时，指定具体版本，尽可能避免缓存过时的问题
...
## 流程调整
1. 已经很久没有qq视频的蚊子腿了，不再尝试获取 vuserid
2. skey检测是否过期用的活动改为新的手机绑定活动

# v22.6.7 2025.6.30
## 活动维护
1. 新增 挑战世界记录 活动
2. 新增 vp挑战赛 活动
3. 云游戏抽奖提示若是最后一周，则每天都弹提示
...
1. 新增 新职业预约 活动
2. 新增 助手春日出游打卡 活动
3. colg签到活动支持自动领取等级奖励以及商城的30天黑钻
4. 手机兑换奖励改为新的id，确保兑换增幅器，并调整判断规则
...
## 其他
1. 同时兼容QQ群二维码是jpg和png的情况，避免下次修改后没注意后缀导致打开失败
...
# v13.6.2 2021.8.12
## 优化
1. 网盘下载接口增加优化机制，可配置仅当网盘中版本比本地版本新时才尝试下载
...
3. 当创建了.no_sync_configs文件后，不仅禁用首次使用时同步配置功能，同时禁用备份配置功能
...
## 代码重构
1. 重构最大化相关函数，使最大化和最小化互不影响。同时允许flag为文件或目录均可
...
## 活动维护
1. 增加漂流瓶每日邀请列表（建议只填小号，因为真的会发消息给对方）(配置项默认不加入配置文件，感兴趣的朋友请自行看源码确认配置方式）
2. 增加马杰洛黑钻送好友邀请列表（建议只填小号，因为真的会发消息给对方）(配置项默认不加入配置文件，感兴趣的朋友请自行看源码确认配置方式）
